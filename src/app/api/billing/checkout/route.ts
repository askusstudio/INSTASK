...
import { findUserById, upsertUser, memoryStore } from '@/lib/prisma';
...
export async function POST(req: Request) {
  try {
    ...
    const planPriceId = body?.planPriceId || process.env.STRIPE_MONTHLY_PRICE_ID || 'price_pro_growth_monthly';
    ...
    // 2. Attach 50% off coupon if they are a first-time subscriber
    const isEligibleForDiscount = !user.isFirstMonthDiscountApplied;
    ...
    const sandboxUrl = `${appUrl}/pricing?sandbox=true&eligible=${isEligibleForDiscount}&userId=${user.id}&planPriceId=${encodeURIComponent(planPriceId)}`;
    ...
    return NextResponse.json({
      ...
      message: 'Sandbox mode active. FIRST50 discount verified.',
    });
  } catch (error: unknown) {
